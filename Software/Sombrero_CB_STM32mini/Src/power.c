...
const double VOLTS_PER_DIV = (3.3 / 4096.0); // REF196 or Vreg+
...
void process_frame (uint16_t offset)
{
  ...
  **********/
  if(ledBlink){HAL_GPIO_WritePin(LED_GPIO_Port, LED_Pin, GPIO_PIN_SET); } // blink the led
  ...
  */
  ...
  for (uint16_t i = 0; i < adc_buff_half_size; i += CTn) // CTn = CT channel quanity.
  {
    ...
    */
    ...
    for (int ch = 0; ch < CTn; ch++)
    {
      ...
      sample_V = adcV_dma_buff[sample_V_index];
      ...
      // Current
      //----------------------------------------
      sample_I = adcI_dma_buff[offset + i + ch];
      ...
      if (sample_I == 4095) channel->Iclipped = true; // much more likely, potentially useful safety information.
      signed_I = sample_I - MID_ADC_READING; // mid-rail removal possible through reading bias? or even necessary?
      ...
      if (dontGiveAMonkeys || (!channel->last_positive_V && channel->positive_V)) { // looking out for a upwards-zero crossing.
        ...
        // debug cycle count and voltage sample.s
        // sprintf(log_buffer, "cycles%d:%ld,sample_V:%d\r\n", ch, channel->cycles, sample_V);
        // debug_printf(log_buffer); 
  ...
  if(ledBlink) { HAL_GPIO_WritePin(LED_GPIO_Port, LED_Pin, GPIO_PIN_RESET);}
