...
bool RFM69_initialize(uint16_t freqBand, uint8_t nodeID, uint16_t networkID)
{
  const uint8_t CONFIG[][2] =
  {
    ...
    /* 0x28 */ { REG_IRQFLAGS2, RF_IRQFLAGS2_PAYLOADREADY }, // writing to this bit ensures that the FIFO & status flags are reset
...
void RFM69_setMode(uint8_t newMode)
{
  ...
  while (rfm_mode == RF69_MODE_SLEEP && (RFM69_readReg(REG_IRQFLAGS1) & RF_IRQFLAGS1_MODEREADY) == 0x00); // wait for ModeReady
...
bool RFM69_canSend()
{
  if (rfm_mode == RF69_MODE_RX && payloadLen == 0 && RFM69_readRSSI(false) < CSMA_LIMIT) // if signal stronger than -100dBm is detected assume channel activity
  {
    RFM69_setMode(RF69_MODE_STANDBY);
    return true;
  }
...
void RFM69_interruptHandler() {
  ...
  if (rfm_mode == RF69_MODE_RX && (RFM69_readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY))
  {
...
bool RFM69_receiveDone()
{
  ...
  if (rfm_mode == RF69_MODE_RX && payloadLen > 0)
  {
    RFM69_setMode(RF69_MODE_STANDBY); // enables interrupts
    return true;
  }
  else if (rfm_mode == RF69_MODE_RX) // already in RX no payload yet
  {
